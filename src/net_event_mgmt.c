...
#define L2_WIFI_SOFTAP_EVENT_MASK (NET_EVENT_WIFI_AP_ENABLE_RESULT | \
...
int init_network_events(void)
{
    ...
    net_mgmt_init_event_callback(&wpa_event_cb, l3_wpa_supp_event_handler, L3_WPA_SUPP_EVENT_MASK);
    ...
    net_mgmt_init_event_callback(&wifi_event_cb, l2_wifi_conn_event_handler, L2_WIFI_CONN_EVENT_MASK);
    net_mgmt_add_event_callback(&wifi_event_cb);
    LOG_DBG("WiFi L2 event handler registered");

    #if IS_ENABLED(CONFIG_UDP_RX_DEV_MODE_SOFTAP)
    /* Initialize SoftAP event callbacks */
    net_mgmt_init_event_callback(&softap_event_cb, l2_wifi_softap_event_handler, L2_WIFI_SOFTAP_EVENT_MASK);
