...
static struct beacon_frame test_beacon_frame = {
	...
	.payload = {
		...
		0X00, 0X11, 
		/* SSID: WIFI_LATENCY_TEST */
		0x57, 0x49, 0x46, 0x49, 0x5F, 0x4C, 0x41, 0x54, 0x45, 0x4E,
		0x43, 0x59, 0x5F, 0x54, 0x45, 0x53, 0x54,
...
int raw_tx_send_packet(uint32_t packet_num)
{
	...
	/* Send the packet */
	ret = sendto(raw_sockfd, test_frame, buf_length, 0,
...
static bool is_raw_tx_packet_beacon(unsigned char *packet, int packet_len)
{
	...
	/* Minimum length check: 802.11 header (24) + beacon fixed fields (12) + SSID IE header (2)*/
	if (packet_len < 38) {
		return false;
	}
...
void raw_rx_dev_monitor_task(void)
{
	...
	while (1) {
		...
		if (is_raw_tx_packet_beacon((unsigned char *)(recv_buffer + RAW_PKT_HDR), recv_len-RAW_PKT_HDR)){
			led_trigger_rx();
			packet_count++;
